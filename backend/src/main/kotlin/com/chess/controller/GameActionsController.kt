...
class GameActionsController(
    ...
    /**
     * Undo the last move (custom games only, creator only)
     */
    @PostMapping("/undo")
    fun undoMove(
    ...
    fun addAnnotation(
        ...
        return if (success) {
            val message = AnnotationAddedMessage(
                gameId = gameId,
                annotation = AnnotationDto.fromDomain(annotation)
            )
    ...
    fun removeAnnotation(
        ...
        return if (success) {
            val message = AnnotationRemovedMessage(
                gameId = gameId,
    ...
    fun clearAllAnnotations(@PathVariable gameId: String): ResponseEntity<Any> {
        ...
        return if (success) {
            messagingTemplate.convertAndSend(
                "/topic/game/$gameId/annotations",
                AnnotationsClearedMessage(gameId)
            )
            ResponseEntity.ok(mapOf("success" to true))
        } else {
            ResponseEntity.status(HttpStatus.NOT_FOUND)
